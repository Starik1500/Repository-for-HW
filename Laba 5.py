...
class Candy:
    def __init__(self, name, mass, amount, price):
        self.name = name
        self.mass = mass
        self.amount = amount
        self.price = price
    ...
    # 2000 = 2 кг, згідно умови завдання
    def ate(self):
...
class Dinner:
    ...
    def find_the_most_expensive_candies(self, numbers):
        sorted_candies = sorted(self.candies, key=lambda x: x.price, reverse=True)
        return sorted_candies[:numbers]


def main():
    Candy1 = Candy("Roshetto", 50, 5, 10)
    Candy2 = Candy("M&M's", 100, 50, 50)
    Candy3 = Candy("Orbit", 50, 5, 25)
    Candy4 = Candy("Eclips", 50, 5, 30)
